...
int process_fat_archive(t_file *file, struct fat_arch *fat_arch, bool print_all)
{
  t_file *new_file;
  char *arch_name;
  uint32_t offset;
...
int handle_fat_archive(t_file *file, struct fat_arch *fat_arch, uint32_t n_arch)
{
  uint32_t i;
  bool print_all;
